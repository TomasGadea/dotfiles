...
# Lazy-load conda: don't run the init hook on every shell startup.
# The real `conda init` block below is only sourced the first time `conda`
# is invoked, which then redefines `conda`/activates as usual.
conda() {
    unset -f conda
    # >>> conda initialize >>>
